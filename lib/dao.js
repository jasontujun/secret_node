                    ...
                    function(err, results) {
                        ...
                        connection.query('INSERT INTO ' + TABLE_SECRET + ' SET ?',
                            ...
                            function (err, result) {
                                ...
                                if (callback)
                                    callback(COMMON_OK, logicId, dfsType,
        ...
        getTableItem(connection, TABLE_SECRET, ['logic_id', 'memory_id'], [secretId, memoryId],
            function(err, secret) {
                ...
                if (secret.dfs) {
                    ...
                    if (callback)
                        callback(COMMON_OK, downloadUrl);
                } else {
                    if (callback)
                        callback(COMMON_OK, secret.url);
                }
            });
    });
};
            ...
            getTableItem(connection, TABLE_SECRET, ['logic_id', 'memory_id'], [secretId, memoryId],
                function (err, secret) {
                    ...
                    if (secret.dfs) {
                        ...
                        if (callback)
                            callback(COMMON_OK, upToken);
                    } else {
                        if (callback)
                            callback(COMMON_OK, null);
                    }
                });
        });
    });
