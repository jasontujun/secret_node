...
var createTableIgnore = "CREATE TABLE IF NOT EXISTS `memory_ignore` (\
...
function initDatabase(callback) {
    pool.getConnection(function(err, connection) {
        ...
        ep.fail(function(err) {
            console.log('[initDatabase] error! err=' + err);
            connection.release();
            if (callback)
                callback(errCode.COMMON_DB);
        });
        ep.after('initdb', tableArray.length, function(list) {
            console.log('[initDatabase] all success! init size=' + list.length);
            connection.release();
            if (callback) {
                callback(errCode.COMMON_OK, list);
            }
        });
        tableArray.forEach(function (tableInitStatement) {
            connection.query(tableInitStatement,
                ep.group('initdb', function(results) {
                    console.log('[initDatabase] init a table success. results=' + results);
                    return results;
                }));
        });
    });
