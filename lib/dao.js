...
var DEATH_THRESHOLD_MIN = 30 * 24 * 60 * 60 * 1000;// 最小死亡检测时长1个月
var DEATH_THRESHOLD_DEFAULT = 365 * 24 * 60 * 60 * 1000;// 默认死亡检测时长1年
var DEATH_THRESHOLD_MAX = 10 * DEATH_THRESHOLD_DEFAULT;// 最大死亡检测时长10年
...
exports.updateAccount = function (userId, user_info, callback) {
    ...
    if (user_info.death_threshold) {
        ...
        if (DEATH_THRESHOLD_MIN <= deathThreshold && deathThreshold <= DEATH_THRESHOLD_MAX) {
                                                            ...
                                                            getTableItem(null, TABLE_MEMORY, 'cover_url', memory.cover_url,
                                                                function(err, result) {
                                                                    if (err && err === errCode.COMMON_DATA_NOT_EXIST) {
                                                                        ...
                                                                        var key = memory.cover_url.replace(dfs.DOMAIN_PUBLIC, '');
                                                                        if (key) {
