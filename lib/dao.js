...
function createUser(conn, user, callback) {
    ...
    if (desArray.length < DES_MIN_SIZE) {
        if (callback)
            callback(errCode.USER_DES_TOO_SHORT);
        return;
    }
    if (desArray.length > DES_MAX_SIZE) {
        if (callback)
...
/**
 * 更新账号信息(包括descriptions, born_time, email, heritage)
 */
                                                ...
                                                function (err, results) {
                                                    ...
                                                    connection.commit(function (err) {
                                                        ...
                                                        for (var i = 0; i < secrets.length; i++) {
                                                            if (secrets[i].dfs) {
                                                                dfs.delete(secrets[i].dfs, secrets[i].dfs_key, function (err) {
                                                                    if (err) {
 ...
 */
exports.updateMemory = function (userId, memoryId, memory, callback) {
    if (!userId || !memory) {
        if (callback)
            callback(errCode.COMMON_PARAM_ILLEGAL);
        return;
    ...
    pool.getConnection(function(err, connection) {
        ...
        var newValue = {};
        if (memory.name) {
            newValue.name = memory.name;
        }
        if (memory.cover_url &&
            !util.isNullOrUndefined(memory.cover_width) &&
            !util.isNullOrUndefined(memory.cover_height)) {
            newValue.cover_url = memory.cover_url;
            newValue.cover_width = Number(memory.cover_width) || 0;
            newValue.cover_height = Number(memory.cover_height) || 0;
        }
        if (!util.isNullOrUndefined(memory.happen_start_time) &&
            !util.isNullOrUndefined(memory.happen_end_time)) {
            var startTime = Number(memory.happen_start_time) || 0;
            var endTime = Number(memory.happen_end_time) || 0;
        ...
        if (memory.heritage) {
            if (memory.heritage === HERITAGE_DEFAULT) {
                newValue.heritage = HERITAGE_DEFAULT;
            } else if (memory.heritage === HERITAGE_PUBLIC) {
                newValue.heritage = HERITAGE_PUBLIC;
            } else if (memory.heritage === HERITAGE_DESTROY) {
                newValue.heritage = HERITAGE_DESTROY;
            } else if(new RegExp("^u").test(memory.heritage)) {
                newValue.heritage = memory.heritage;
            }
        }
                                ...
                                function (err, results) {
                                    ...
                                    connection.query('DELETE FROM ' + TABLE_IGNORE + ' WHERE gift_id = ?',
                                        ...
                                        function (err, results) {
                                            ...
                                            // 第三步：删除无用的种子user(PUBLIC_SCOPE的种子用户)
                                            connection.query('DELETE u FROM ' + TABLE_USER + ' u\
                                            LEFT JOIN ' + TABLE_GIFT + ' g ON u.state=? AND u.logic_id=?\
                                            AND u.logic_id = g.receiver_id AND g.take_time=0\
                                            WHERE g.logic_id IS NULL AND u.logic_id=?',
                                                [USER_STATE_SEED, giftItem.receiver_id, giftItem.receiver_id],
                                                function (err, results) {
                                                    if (err) {
                                                        connection.rollback(function() {
                                                            connection.release();
                ...
                function(err, secret) {
                    ...
                    connection.query('DELETE FROM ' + TABLE_SECRET + ' WHERE logic_id = ? AND memory_id = ?',
                        ...
                        function (err, results) {
                            ...
                            if (secret.dfs) {
                                dfs.delete(secret.dfs, secret.dfs_key, function (err) {
                                    if (err) {
                                        // TODO 如果DFS删除失败，记录错误日志，未删除的无用文件
                                    }
