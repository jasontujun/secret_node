                                ...
                                connection.query('INSERT INTO ' + TABLE_MEMORY + ' SET ?',
                                    {
                                        logic_id : memoryId,
                                        name : memory.name,
                                        author_id : boxItem.sender_id,
                                        author_name : boxItem.sender_name,
                                        owner_id : receiverId,
                    ...
                    function(err, results) {
                        ...
                        connection.query('INSERT INTO ' + TABLE_SECRET + ' SET ?',
                            ...
                            function (err, result) {
                                ...
                                if (callback) {
                                    var key = createSecretDfsKey(logicId);
                                    var upToken = dfs.genSecretUploadToken(dfsType, key);
                                    callback(errCode.COMMON_OK, logicId, dfsType, key, upToken);
                                }
                            });
                    });
            });
        ...
        getTableItem(connection, TABLE_MEMORY, 'logic_id', memoryId, function(err, memory) {
            ...
            getTableItem(connection, TABLE_SECRET, ['logic_id', 'memory_id'], [secretId, memoryId],
                function(err, secret) {
                    ...
                    if (secret.dfs !== dfsType || !secret.url) {
                        // 已经设置了第三方公开的url，或者回调的dfs类型和secret指定的dfs类型不同，则报错
                        connection.release();
                        if (callback)
                            callback(errCode.COMMON_DENY);
                    }
